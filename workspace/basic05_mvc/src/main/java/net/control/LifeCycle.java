...
/*
 	...
 	● HttpServlet 생명주기-----------------------------
 		init()	서블릿이 호출되면 1번만 호출
 			-> service()
 			-> service()
 			-> service()  사용자가 요청할때마다 계속 호출 () 
 			-> service()  doGet 과 doPost()를 구분하는 역활
 			-> service()
 		destroy()	서버가 중지되면 1번만 호출 		 	  
 	------------------------------------------------  
 	  
 	● LifeCycle 서블릿을 /WEB-INF/web.xml에 등록해야 함
 	● 결과확인 http://localhost:9090/basic05_mvc/life.doH
 */
...
public class LifeCycle extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//mothod=get방식으로 요청하면 service()함수가 호출함
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//mothod=post방식으로 요청하면 service()함수가 호출함
